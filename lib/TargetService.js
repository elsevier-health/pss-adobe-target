...
/**
 * Extracts the experience from the activity offer defined in adobe target. The value will be 'control' or 'variation'.
 * If the experiment isn't running or there is an error, undefined will be returned and an error will be logged.
 * @param response  see {$link https://developers.adobetarget.com/api/delivery-api/#section/Single-or-Batch-Delivery/Batched-Delivery-Call} for documentation on AT getOffers response.
 * @param mboxName string - This is the main mbox location defined in Adobe Target
 * @returns String experience {'control', 'variation', or undefined}
 */
    ...
    try {
        ...
        for (const mbox of mboxes) {
            if (mbox.name === mboxName &&
                ...
                // This assumes the Adobe Target offer was created with the best practice pattern defined in the README. { "experience": variation_name_here }
                // We should only create a single offer per experience.
                experience = mbox.options[0].content.experience;
            }
        }
    }
    catch (error) {
        logger.error("Unexpected Adobe Target activity response received for mbox = " + mboxName);
...
// TODO - Add getExperience https://elsevier.atlassian.net/browse/AMP-4737
...
const getActivitiesMiddleware = (req, res, next) => {
    ...
    const sessionId = getUserId(req.session);
